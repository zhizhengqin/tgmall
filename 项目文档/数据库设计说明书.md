...
> **文档版本**：V2.0  
> **编制日期**：2026 年 6 月 9 日
> **V2 变更**：新增 AdminUser 表（用户名密码登录）；Merchant 表不再接收新入驻，由 defaultMerchant 中间件自动创建默认商户  
> **对应阶段**：软件工程阶段二 —— 系统设计  
...
| **查询性能** | MVP 阶段支撑 10 万用户 + 500 商家 + 10 万商品 + 日均 500 单的查询需求 |
...
| **外键** | `{关联表单数名}_id` | `user_id`、`merchant_id` |
...
| **外键约束名** | `fk_{表名}_{关联表名}` | `fk_products_merchant` |
...
| 短文本（≤200 字符） | `VARCHAR(n)` | 店铺名、人名、手机号 |
...
erDiagram
    ...
    merchants ||--o{ products : "上架"
    merchants ||--o{ orders : "收到"
    merchants ||--o{ coupons : "发放"
...
| `merchants` | `products` | 一对多 | 1 : 0..N | 一个商家可以上架多个商品 |
| `merchants` | `orders` | 一对多 | 1 : 0..N | 一个商家可以收到多个订单 |
| `merchants` | `coupons` | 一对多 | 1 : 0..N | 一个商家可以发放多张优惠券 |
...
**用途**：存储商家店铺信息。商家需提交入驻申请，审核通过后 `status` 变为 `active`。

| 字段名 | 类型 | 约束 | 默认值 | 说明 |
|--------|------|------|--------|------|
| `id` | `UUID` | **PK** | `gen_random_uuid()` | 商家唯一标识 |
| `name_km` | `VARCHAR(200)` | `NOT NULL` | — | 店铺名称（高棉语，必填） |
| `name_en` | `VARCHAR(200)` | — | `NULL` | 店铺名称（英语） |
| `owner_name` | `VARCHAR(100)` | `NOT NULL` | — | 店主姓名 |
| `phone` | `VARCHAR(20)` | **UK, NOT NULL** | — | 联系电话（+855 格式） |
| `address` | `TEXT` | `NOT NULL` | — | 店铺地址 |
| `category` | `VARCHAR(50)` | `NOT NULL, CHECK` | — | 主营品类 |
| `description` | `TEXT` | — | `NULL` | 店铺简介 |
| `status` | `VARCHAR(20)` | `NOT NULL, CHECK` | `'pending'` | 审核状态 |
| `commission_rate` | `DECIMAL(3,1)` | `NOT NULL` | `3.0` | 平台佣金率（百分比，如 3.0 = 3%） |
| `reject_reason` | `TEXT` | — | `NULL` | 审核驳回原因（三语） |
| `telegram_id` | `BIGINT` | — | `NULL` | 关联的 Telegram 用户 ID（用于商家登录） |
...
```sql
-- 创建商家表
-- 说明：商家提交入驻申请后插入，审核通过后 status = 'active'
-- 审核通过前 telegram_id 可为空，通过后绑定商家 Telegram 账号
CREATE TABLE merchants (
  ...
  commission_rate DECIMAL(3,1) NOT NULL DEFAULT 3.0
                  CHECK (commission_rate >= 0 AND commission_rate <= 20),
  reject_reason   TEXT,
  telegram_id     BIGINT,
  created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
...
| `merchant_id` | `UUID` | **FK → merchants, NOT NULL** | — | 归属商家 |
...
```
                   ┌──────────────┐
                   │ pending_payment│ ← 订单创建时的初始状态
                   └──────┬───────┘
                          │
              ┌───────────┼───────────┐
              │ 用户支付成功 │           │ 超时 15min / 用户取消
              ▼           │           ▼
        ┌──────────┐      │    ┌───────────┐
        │   paid   │      │    │ cancelled │ ← 库存释放，优惠券退还
        └────┬─────┘      │    └───────────┘
             │            │
             │ 商家发货     │
             ▼            │
...
CREATE TABLE orders (
  ...
  payment_status    VARCHAR(20) NOT NULL DEFAULT 'pending'
                    CHECK (payment_status IN ('pending', 'processing', 'success', 'failed')),
...
**价格快照说明**：`price_usd` 和 `price_khr` 存的是**下单那一刻**的商品价格，而不是引用 `products` 表的当前价格。这样即使商家后续调价，历史订单金额不受影响。

**DDL**：

```sql
-- 创建订单商品表
-- 说明：价格快照保证历史订单的准确性。即使商家后期调价，订单金额不变。
...
**用途**：存储优惠券模板。支持平台券（`merchant_id = NULL`）和商家券（`merchant_id` 指向具体商家）。

| 字段名 | 类型 | 约束 | 默认值 | 说明 |
|--------|------|------|--------|------|
| `id` | `UUID` | **PK** | `gen_random_uuid()` | 优惠券唯一标识 |
| `merchant_id` | `UUID` | **FK → merchants** | `NULL` | 发券商家（NULL = 平台券） |
...
```sql
-- 创建优惠券表
-- 说明：merchant_id 为 NULL 表示平台券，不为 NULL 表示商家券。
...
| `merchant_id` | `UUID` | **FK → merchants** | `NULL` | 接收通知的商家 |
...
| type 值 | 说明 | 接收方 |
|---------|------|--------|
| `order_created` | 订单已创建 | 消费者 + 商家 |
| `payment_success` | 支付成功 | 消费者 + 商家 |
| `order_shipped` | 订单已发货 | 消费者 |
| `order_completed` | 订单已完成 | 消费者 |
| `order_cancelled` | 订单已取消 | 消费者 + 商家 |
| `merchant_approved` | 商家审核通过 | 商家 |
| `merchant_rejected` | 商家审核驳回 | 商家 |
...
-- user_id 和 merchant_id 至少有一个非空（通知至少有一个接收方）。
...
**种子数据**：启动时自动创建默认管理员 `admin`，密码由环境变量 `ADMIN_PASSWORD` 或默认 `admin123` 生成。
...
| `merchants` | 4 | 1 (`phone`) | 0 | 0 | 0 |
| `products` | 8 | 0 | 2 | 0 | 1 (GIN trgm) |
| `orders` | 7 | 1 (`order_number`) | 2 | 2 | 0 |
| `order_items` | 2 | 0 | 0 | 0 | 0 |
| `coupons` | 3 | 0 | 0 | 1 | 0 |
| `user_coupons` | 2 | 1 (`user_id+coupon_id`) | 0 | 0 | 0 |
| `addresses` | 2 | 0 | 0 | 1 | 0 |
| `notifications` | 4 | 0 | 0 | 1 | 0 |
| **合计** | **36** | **4** | **4** | **5** | **1** |
...
| **商家订单** | `WHERE merchant_id='xxx' AND status='paid' ORDER BY created_at DESC` | `idx_orders_merchant`（前导列匹配） |
...
| 商家看板数据 | `merchant_dashboard:{id}` | 5 分钟 | 首次查询写入 |
...
1.  users              ← 无外键依赖，最先建
2.  merchants          ← 无外键依赖
3.  coupons            ← 依赖 merchants (FK, 可空)
4.  products           ← 依赖 merchants (FK)
5.  orders             ← 依赖 users + merchants + coupons (FK)
6.  order_items        ← 依赖 orders + products (FK)
7.  user_coupons       ← 依赖 users + coupons (FK)
8.  addresses          ← 依赖 users (FK)
9.  notifications      ← 依赖 users + merchants (FK, 可空)
10. orders_archive     ← 无外键依赖（归档表，结构与 orders 相同）
```
...
-- 插入测试商家（已审核通过）
INSERT INTO merchants (name_km, name_en, owner_name, phone, address, category, status, commission_rate)
VALUES
  ('ហាង សំលៀកបំពាក់ សុភាព', 'Sopheap Fashion', 'Sopheap Kong', '+85512345001',
   'No. 123, Street 456, BKK1, Phnom Penh', 'fashion', 'active', 3.0),
  ('ហាង គ្រឿងសម្អាង ស្រីស្អាត', 'Srey Saat Beauty', 'Chantrea Mey', '+85512345003',
   'No. 789, Russian Blvd, Toul Kork, Phnom Penh', 'beauty', 'active', 3.0);

-- 插入测试商品
INSERT INTO products (merchant_id, name_km, name_en, name_zh, price_usd, price_khr, stock, category, images)
SELECT
  m.id,
...
FROM merchants m WHERE m.name_en = 'Sopheap Fashion';
